...
[[https://github.com/yt-dlp/yt-dlp][yt-dlp]] which would enable you to play remote videos. I am not encouraging its
use, but simply making you aware that this package exists and serves that
purpose :).

You can use this package even without Emacs, as it is simply a very powerful video player.
...
remote videos. Again, I am not encouraging its use, but simply making you aware that
this version of the app exists and serves that purpose :).

You can use this app even without Emacs, as it is simply a very powerful video player.
...
local and remote videos using MPV.
...
Android in my case had a problem of playing a video through MPV using the above
...
together and send commands to the mpv-android app and let it play videos.

To do this, it is necessary to advise the `mpv-play` command from Emacs in order
to use a different set of commands when requesting to play a video on
